...
const generateSchema = z.object({
  source_text: z
    .string()
    .min(
      VALIDATION_CONSTANTS.SOURCE_TEXT_MIN_LENGTH,
      `Text must be at least ${VALIDATION_CONSTANTS.SOURCE_TEXT_MIN_LENGTH} characters long`
    )
    .max(
      VALIDATION_CONSTANTS.SOURCE_TEXT_MAX_LENGTH,
      `Text must not exceed ${VALIDATION_CONSTANTS.SOURCE_TEXT_MAX_LENGTH} characters`
    ),
