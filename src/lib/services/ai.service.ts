...
export class AIService {
  ...
  async generateFlashcards({
    sourceText,
    model = AI_CONFIG.openrouter.defaultModel as OpenRouterModel,
  }: FlashcardGenerationPrompt): Promise<CreateFlashcardDTO[]> {
    // Validate source text length
    if (
      sourceText.length < VALIDATION_CONSTANTS.SOURCE_TEXT_MIN_LENGTH ||
      sourceText.length > VALIDATION_CONSTANTS.SOURCE_TEXT_MAX_LENGTH
    ) {
      throw new Error(
        `Source text must be between ${VALIDATION_CONSTANTS.SOURCE_TEXT_MIN_LENGTH} and ${VALIDATION_CONSTANTS.SOURCE_TEXT_MAX_LENGTH} characters`
      );
    }
